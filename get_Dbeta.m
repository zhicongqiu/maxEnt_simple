function Dbeta = get_Dbeta(data,Fs_multiclass,w,data_GTT_temp,...
			   label_temp,l,active_set,num,a_u,mode)
...
for i=1:size(Fs_multiclass,1)
    if label_temp(i)~=2 %labeled class
        ...
        if mode==2||mode==3||mode==4
            ...
            for j=1:length(temp)
                if temp(j)==l
                    sum2 = sum2-(1-Fs_multiclass(i,l))*data(i);
                else
                    sum2 = sum2+Fs_multiclass(i,l)*data(i);
                end
            end
        end
    end
